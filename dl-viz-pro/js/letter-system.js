...
const LETTER_CONFIG = {
  spring: {
    hover: { stiffness: 0.35, damping: 0.72, mass: 1.0 },
    body: { stiffness: 0.20, damping: 0.78, mass: 1.0 },
    scared: { stiffness: 0.38, damping: 0.62, mass: 1.1 },
    snake: { stiffness: 0.10, damping: 0.88, mass: 1.0 },
    emotion: { stiffness: 0.48, damping: 0.72, mass: 1.0 },
    breath: { stiffness: 0.05, damping: 0.94, mass: 1.0 },
  },
  interactions: {
    hoverElevation: 10,
    hoverScale: 1.10,
    scaredTriggerDist: 80,
    scaredRecoveryDist: 130,
    scaredSpeedThreshold: 18,
    pupilMaxOffset: 4,
    pupilTrackingFactor: 0.012,
    gradientFar: 400,
    gradientMid: 280,
    gradientNear: 140,
    tapDelay: 150,
  ...
  snake: { speedThreshold: 28, consecutiveFrames: 5, cooldown: 5000, maxDuration: 4000, slowThreshold: 15 },
  lazy: {
    checkInterval: 5000,
    minInterval: 10000,
    actions: {
  ...
  chorus: { tapCount: 5, tapWindow: 3000, staggerDelay: 120, jumpHeight: -18 },
...
function createParticleEngine(canvas) {
  ...
  const PARTICLE_TYPES = {
    ...
    confetti: { color: '#FF69B4', size: 5, life: 1800, gravity: 0.04, shape: 'rect' },
  };
...
class LetterCharacter {
  ...
  triggerScared(directionX) {
    ...
    this.springs.x.target = pushDir * 15;
    this.springs.y.target = -12;
    this.springs.rotation.target = pushDir * 10;
    this.springs.scaleX.target = 0.85;
  ...
  triggerPress() {
    ...
    this.springs.scaleX.target = 1.18;
    this.springs.scaleY.target = 0.82;
  }

  triggerRelease() {
    this.isPressed = false;
    this.springs.scaleX.target = 0.90;
...
class LetterSystem {
  ...
  _applyChristmasDecoration() {
    ...
    this._addHolidayStyle('.holiday-christmas', {
      background: 'linear-gradient(180deg, rgba(255,0,0,0.05) 0%, rgba(0,100,0,0.05) 100%)',
    });
  }
  ...
  _applyNewYearDecoration() {
    ...
    this._addHolidayStyle('.holiday-newyear', {
      background: 'linear-gradient(180deg, rgba(255,215,0,0.08) 0%, rgba(255,100,0,0.05) 100%)',
    });
  }
  ...
  _applySpringFestivalDecoration() {
    ...
    this._addHolidayStyle('.holiday-springfestival', {
      background: 'linear-gradient(180deg, rgba(255,50,50,0.08) 0%, rgba(255,200,0,0.05) 100%)',
    });
  }
