...
function toggleFavorite(modelId) {
  // 防止快速点击
  if (favoriteLocks.has(modelId)) return;
  favoriteLocks.add(modelId);

  requireLogin(() => {
    const idx = userState.favorites.indexOf(modelId);
    if (idx === -1) {
      userState.favorites.push(modelId);
  ...
  // 无论是否登录，都确保锁被释放
  setTimeout(() => favoriteLocks.delete(modelId), 300);
}
...
function updateFavoriteButtons() {
  document.querySelectorAll('.fav-btn').forEach(btn => {
    ...
    const isFav = userState.favorites.includes(id);
    if (isFav) {
      btn.classList.add('active');
...
function navigate(view, params = {}) {
  ...
  if (activeView && !prefersReducedMotion) {
    ...
    // 安全回退：如果动画未触发（如display:none），300ms后强制切换
    setTimeout(() => {
...
function _showView(view, params) {
  requestAnimationFrame(() => {
    ...
    switch (view) {
      ...
      case 'model':
        document.getElementById('modelView').classList.add('active');
      ...
      case 'learningPath':
        document.getElementById('learningPathView').classList.add('active');
        renderLearningPath();
        break;
    }
...
function renderModelGrid(data) {
  ...
  grid.innerHTML = data.map((m, index) => {
    ...
    return `
    <div class="model-card ${compareState.enabled ? 'compare-mode' : ''}" onclick="showDetail(${m.id})"
         style="animation: fadeInUp 0.4s var(--ease-out) ${index * 0.05}s both;"
         role="button" tabindex="0"
         aria-label="${escapeHtml(m.name)}"
         onkeydown="if(event.key==='Enter') showDetail(${m.id})">
...
function renderModelPage(modelName) {
  ...
  if (!m) {
    document.getElementById('modelPageBody').innerHTML = '<div class="model-page-placeholder"><p>模型未找到</p></div>';
    return;
  }
...
function showDetail(id) {
  ...
  body.innerHTML = `
    ...
    <!-- 中间：两列信息网格 -->
    ${(m.author || m.organization || m.parameters || m.datasets || m.performance) ? `
    <div class="detail-info-grid" style="margin-bottom: 20px;">
      ${m.author ? `<div class="detail-info-item"><div class="label">作者</div><div class="value">${escapeHtml(m.author)}</div></div>` : ''}
      ${m.organization ? `<div class="detail-info-item"><div class="label">机构</div><div class="value">${escapeHtml(m.organization)}</div></div>` : ''}
      ${m.architecture ? `<div class="detail-info-item"><div class="label">架构</div><div class="value">${escapeHtml(m.architecture)}</div></div>` : ''}
      ${m.parameters ? `<div class="detail-info-item"><div class="label">参数量</div><div class="value">${escapeHtml(m.parameters)}</div></div>` : ''}
    </div>
    ` : ''}
    ...
    <div class="detail-desc" style="margin-bottom: 20px; border-left-color: var(--accent-secondary);">
      ...
      ${addTermTooltips(m.keyInnovation)}
    </div>
    ` : ''}

    <!-- 标签区域：pill形标签 -->
    ${m.tags && m.tags.length ? `
    <div class="detail-tags" style="margin-bottom: 20px;">
      ${m.tags.map(t => `<span class="tag">${escapeHtml(t)}</span>`).join('')}
    </div>
    ` : ''}
    ...
    <div class="detail-actions">
      ${m.paperUrl ? `<a href="${escapeHtml(m.paperUrl)}" target="_blank" rel="noopener" class="btn btn-secondary">论文链接</a>` : ''}
      <button class="btn btn-primary" onclick="closeModal('detailModal'); navigate('model', {name: '${escapeHtml(m.name).replace(/'/g, "\\'")}'})">查看完整详情 →</button>
    </div>
    ` : ''}
  `;
  ...
  try {
    ...
    if (!state.models || state.models.length === 0) {
      ...
      document.getElementById('app').innerHTML = `
        <div style="display:flex;flex-direction:column;align-items:center;justify-content:center;min-height:60vh;color:var(--text-secondary);padding:20px;max-width:500px;margin:0 auto;">
    ...
    const skeleton = document.getElementById('skeletonOverlay');
    if (skeleton) skeleton.style.display = 'none';
    document.getElementById('app').innerHTML = '<div style="display:flex;align-items:center;justify-content:center;min-height:60vh;color:var(--text-secondary);"><p>加载失败，请刷新页面重试</p></div>';
  }
}
